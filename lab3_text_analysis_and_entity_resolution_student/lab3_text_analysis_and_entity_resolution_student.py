...
def countTokens(vendorRDD):
    ...
    return vendorRDD.map(lambda (a,b):(1,len(b))).reduceByKey(lambda a,b:a+b).collect()[0][1]
...
def tf(tokens):
    ...
    """
    tokenLength=float(len(tokens))
    #tokensRDD = sc.parallelize(tokens)
    #tokensCountRDD = tokensRDD.map(lambda x:(x,1)).reduceByKey(lambda a,b:a+b)
    #return dict(tokensCountRDD.map(lambda (k,v):(k,v/tokenLength)).collect())
    emptyDict={}
    for item in tokens : 
        if item not in emptyDict : emptyDict[item]=1 
        
        else : emptyDict[item] += 1
    tfIdfDict = {k:v/tokenLength for k,v in emptyDict.items()}
    return tfIdfDict
...
def idfs(corpus):
    ...
    uniqueTokens = sc.parallelize(corpus.map(lambda (k,v):(1,list(set(v)))).reduceByKey(lambda a,b:a+b).values().take(1)[0])
    tokenCountPairTuple = uniqueTokens.map(lambda x:(x,1)).reduceByKey(lambda a,b:a+b)
    tokenSumPairTuple = tokenCountPairTuple.map(lambda (k,v):(k,(N/float(v))))
...
def tfidf(tokens, idfs):
    ...
    #tfIdfDict =sc.parallelize(tfs.items()).map(lambda (k,v):(k,v*idfs[k])).collectAsMap()
    return tfIdfDict
...
def dotprod(a, b):
    ...
    """
    #{k:v*idfs[k] for k,v in tfs.items()}
    return sum({k: a[k] * b[k] for k in set(b) & set(a)}.values())
    #aANDbRDD = sc.parallelize(a.items()).join(sc.parallelize(b.items())) 
    #if aANDbRDD.isEmpty() :
    #    return 0
    #else :
    #    return (aANDbRDD.map(lambda (k,v):(1,v[0]*v[1]))
    #      .reduceByKey(lambda a,b:a+b).values().take(1)[0])
...
def norm(a):
    ...
    """
    return math.sqrt(sum({k: a[k] * a[k] for k in set(a) & set(a)}.values()))
    #return math.sqrt((sc.parallelize(a.items())
          #.join(sc.parallelize(a.items()))
         # .map(lambda (k,v):(1,v[0]*v[1])).reduceByKey(lambda a,b:a+b).values().take(1)[0]))
...
#print(sims.count())
#print(goldStandard.count())
#print(sims.take(2))
trueDupsRDD = (sims
               .join(goldStandard))
#print(trueDupsRDD.count())
trueDupsCount = trueDupsRDD.count()
avgSimDups = trueDupsRDD.map(lambda (k,v):(1,v[0])).reduceByKey(lambda a,b:a+b).values().collect()[0]/float(trueDupsCount)
#print(avgSimDups)
...
#print(nonDupsCount)
avgSimNon = nonDupsRDD.map(lambda (k,v):(1,v)).reduceByKey(lambda a,b:a+b).values().collect()[0]/float(nonDupsCount)
...
# TODO: Replace <FILL IN> with appropriate code
amazonFullRecToToken = amazon.map(lambda (a,b):(a,tokenize(b)))
googleFullRecToToken = google.map(lambda (a,b):(a,tokenize(b)))
...
def invert(record):
    ...
    pairs = [(v,record[0]) for v in record[1]]
    #print(len(pairs.items()))
    return pairs

#print(len(invert(amazonWeightsRDD.map(lambda (k,v):(k,v.keys())).take(1))))
amazonInvPairsRDD = (amazonWeightsRDD
                  .map(lambda(k,v):(k,v.keys())).flatMap(invert)
                  .cache())
...
commonTokens = (amazonInvPairsRDD
                .join(googleInvPairsRDD).map(swap)
                .groupByKey()
                .cache())
